        ...
        **kwargs: Any,
        ...
        try:
            ...
            if _is_blocked_device(path):
                return f"Error: Reading {path} is blocked (device path that could hang)."

            fp = _resolve_path(path, self._workspace, self._allowed_dir)
            if _is_blocked_device(fp):
                return f"Error: Reading {fp} is blocked (device path that could hang)."
            if not fp.exists():
            ...
            result += (
                f"\n\n(Extracted {len(selected_pages)} page(s) from {path}; "
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not blocks:
                return f"(No extractable text in DOCX: {path})"
            ...
            result += f"\n\n(Extracted {len(blocks)} block(s) from {path})"
            return result
        except ImportError:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            for row_index, row in enumerate(
                ...
                row_count += 1
                if row_count > self._DEFAULT_MAX_ROWS:
                    truncated = True
                    break
            ...
            if truncated:
                result += (
                    f"\n\n(Stopped after {self._DEFAULT_MAX_ROWS} rows. "
                    "Provide a smaller range=... for a focused read.)"
                )
            result += f"\n\n(Extracted {min(row_count, self._DEFAULT_MAX_ROWS)} row(s) from {path})"
            return result
        except ImportError:
